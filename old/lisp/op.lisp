...
(defmacro def-binary-op (name args type raw-op &optional (coerce 'identity) (getter (symb type '-value)))
  `(defun ,name ,args
     (declare (,type ,@args))
     (,type (,coerce 
...
;;(def-binary-op @eq (x2 x1) %int eq identity identity)
(defun @eq (x1 x2)
  (%int (bool-to-int (eq x1 x2))))
